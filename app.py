...
model= keras.models.load_model("assets\VIT_Weights")

model1= keras.models.load_model("assets/vit190/kaggle/working/VIT_Wei")
@app.route("/")
def homepage():
...
@app.route('/word1')
def wordcanvas():
    return render_template("wordcanvas.html")
...
def predictword():
    if request.method == "POST":
        ...
        bbox_path = os.path.join('static/images/', 'bbox.png')
        plt.imshow(img,cmap='gray')
        plt.savefig(bbox_path)
        print(bbox_path)
        # cv2.imread(bbox_path)/
        # cv2.waitKey(0)
        return render_template('predict.html', num=ans,num1="Word",img_path = bbox_path,flag="yes")
    else:
        return redirect("/")
...
def something():
    if request.method == "POST":
        ...
        prediction=model1.predict(test_sample_image)
        predicted_class=np.argmax(prediction)
        print(predicted_class)          
        return render_template('predict.html', num=str(unicode_mapping_df.loc[predicted_class][1]),num1="Character")
    else:
...
def some():
    if request.method == "POST":
        ...
        cnts = sorted(cnt_t, key=lambda b: b[0][0][0], reverse=False)
        # fig=plt.figure()
        # plt.imshow(cv2.drawContours(img, cnts, -1, (0,255,0), 3),cmap='gray',vmin=0)
